...
class _AddProductScreenState extends State<AddProductScreen> {
  ...
  Future<void> _fetchShopName() async {
    final shopDoc = await _firestore.collection('vendors').doc(widget.user.uid).get();
    if (shopDoc.exists) {
      final shopData = shopDoc.data();
      if (shopData != null && shopData.containsKey('shopName')) {
        setState(() {
          _shopName = shopData['shopName'] ?? 'Unknown Shop';
        });
      }
    }
  }
  ...
  void _saveProduct() async {
    ...
    try {
      ...
      final productData = {
        ...
        'category': _selectedCategory,
        'shopName': _shopName,
        'image': imageUrl ?? widget.existingProductData?['image'],
      };
